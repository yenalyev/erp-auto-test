...
def crew_relocation_cases() -> list[Case]:
    amt = "ISSUE_AMOUNT=20; stock sender=100."
    return [
        mk("TC-CREW-REL-001", FEAT_CREW_ISSUE, "AC-01", "Видача UNIT→CREW — AUTO_FINISHED, stock ±",
           "Ресурси списуються з підрозділу і зараховуються на баланс екіпажу одразу (без підтвердження).",
           ...
           steps=[
               ...
               ("POST send UNIT → CREW", "state=AUTO_FINISHED"),
               ("Перевірити stock", "sender −20; crew +20"),
           ]),
        mk("TC-CREW-REL-002", FEAT_CREW_ISSUE, "AC-02", "Журнал видач — фільтр productIds + senderIds",
           "Видача на екіпаж з'являється у журналі «Видано» відправника.",
           preconditions=PRE_CREW, tags="crew,relocations,api", role_name=ROLE_OWNER,
           steps=[
               ("Виконати send UNIT→CREW", "relocationId"),
               ("GET /relocations sent history (UI query params)", "relocationId у content"),
           ]),
        ...
        mk("TC-CREW-REL-004", FEAT_CREW_ISSUE, "AC-04", "Multi-resource send (2+ позиції)",
           "Одна видача може містити кілька ресурсів; stock оновлюється для кожного.",
           preconditions=PRE_CREW, tags="crew,relocations,api", role_name=ROLE_OWNER,
           steps=[
               ("Підготувати 2 ресурси з stock", "resource A, B"),
               ("POST send з 2 рядками", "AUTO_FINISHED"),
               ("Перевірити stock обох", "sender −N; crew +N для кожного"),
           ]),
        ...
        mk("TC-CREW-REL-006", FEAT_CREW_ISSUE, "AC-06", "Видача з PRODUCTION sender",
           "Виробнича локація може бути відправником видачі на екіпаж.",
           preconditions=PRE_CREW, tags="crew,relocations,api", role_name=ROLE_OWNER,
           test_data="Ephemeral PRODUCTION child; stock; crew recipient.",
           steps=[
               ("POST send PRODUCTION → CREW", "AUTO_FINISHED"),
               ("Перевірити stock PRODUCTION і crew", "±amount"),
           ]),
        mk("TC-CREW-REL-007", FEAT_CREW_ISSUE, "AC-07", "Журнал отримувача (crew storage)",
           "Видача видима в received history з точки зору crew storage.",
           preconditions=PRE_CREW, tags="crew,relocations,api", role_name=ROLE_OWNER,
           steps=[
               ("Send UNIT→CREW", "relocationId"),
        ...
        mk("TC-CREW-REL-008", FEAT_CREW_ISSUE, "AC-08", "Multi-item — кожна позиція ≤ stock",
           "Валідація по кожному рядку форми: сума не перевищує залишок.",
           preconditions=PRE_CREW, tags="crew,relocations,api", role_name=ROLE_OWNER,
           steps=[("POST send 2 resources у межах stock", "HTTP 200; stock коректний")]),
        mk("TC-CREW-REL-009", FEAT_CREW_ISSUE, "AC-09", "UNIT→CREW прихований для ACCOUNTANT",
           ...
           role_name=ROLE_ACC,
           steps=[
               ("OWNER_1: send UNIT→CREW", "relocationId"),
...
def crew_inventory_cases() -> list[Case]:
    return [
        mk("TC-CREW-INV-001", FEAT_CREW_INV, "AC-01", "Звіт STOCK — залишки на екіпажі після видачі",
           ...
           steps=[
               ("OWNER_1: send UNIT→CREW amount=20", "AUTO_FINISHED"),
...
def crew_ui_cases() -> list[Case]:
    ...
    return [
        ...
        mk("TC-UI-CREW-002", FEAT_CREW_ISSUE, "AC-12", "UI — happy path видачі на екіпаж",
           "Повний сценарій: підрозділ → екіпаж → ресурс → кількість → підтвердити → рядок у «Видано».",
           priority="CRITICAL", preconditions=pre, tags="crew,ui", role_name=ROLE_OWNER, layer="UI",
           ...
           steps=[
               ...
               ("Підтвердити", "Редірект на журнал"),
               ("Вкладка «Видано» — знайти ресурс", "Рядок присутній"),
           ]),
        ...
        mk("TC-UI-CREW-004", FEAT_CREW_INV, "AC-11", "UI — залишки екіпажу на /unit-management",
           ...
           steps=[
               ("Виконати видачу UNIT→CREW (API або UI)", "stock на crew"),
...
def extra_features() -> list[tuple]:
    return [
        ...
        (FEAT_CREW_ISSUE, FEAT_CREW_ROOT, "Видача ресурсів на екіпаж",
         "OWNER видає з підрозділу на CREW (AUTO_FINISHED); журнал; RBAC; UI форма «Видати на екіпаж».",
         "WMS", "CRITICAL", "2", "1"),
...
def extra_acceptance_criteria() -> list[tuple]:
    ...
    crew_issue = [
        ("AC-01", "Видача UNIT→CREW → AUTO_FINISHED; stock sender −N, crew +N"),
        ("AC-02", "Видача видима в журналі відправника (sent history)"),
        ("AC-03", "Негатив: amount > stock → HTTP 400"),
        ("AC-04", "Multi-resource send оновлює stock кожного ресурсу"),
        ("AC-05", "Підрозділ поза CREWS region не може send на чужий crew"),
        ("AC-06", "PRODUCTION sender може видавати на CREW"),
        ("AC-07", "Видача видима в received history crew storage"),
        ("AC-08", "Multi-item: кожна позиція ≤ stock"),
        ("AC-09", "UNIT→CREW прихований для ACCOUNTANT у GET /relocations"),
        ("AC-10", "totalRemovedResources на sender збільшується після видачі"),
        ("AC-11", "UI: кнопка «Видати на екіпаж» видима для member з CREWS"),
        ("AC-12", "UI: happy path форми видачі на екіпаж"),
