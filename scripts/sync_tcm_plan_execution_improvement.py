...
core.ACCEPTANCE_CRITERIA = [
    ...
    ("AC-02", "Аркуші За планом і Поза планом містять поточні видимі рядки, включно зі згорнутим out-of-plan: За планом зберігає чинні 6 колонок; Поза планом має Продукт | Категорія | Зроблено | Од. вимір | На складі (<дата>); Розбір лишається Продукт | Кількість і не фільтрується за категорією, пошуком та обраними. Аркуші Поза планом і Розбір створюються завжди. Без відфільтрованих out-of-plan рядків Поза планом містить лише погоджений header; без даних розбору вибраної локації та періоду Розбір порожній."),
...
core.CASES = [
    ...
    case("TC-API-PLANEXEC-002", "AC-02", "Backend Excel", "Експортувати execution без категорійного фільтра та з двома категоріями, одна з яких не має рядків виконання.", "Валідний XLSX завжди має три аркуші: чинні 6 колонок За планом; Продукт | Категорія | Зроблено | Од. вимір | На складі (<дата>) у Поза планом; Продукт | Кількість у Розбір; поточну дату й потрібний ресурс. Без відфільтрованих out-of-plan рядків Поза планом містить лише погоджений header; Розбір має однаковий вміст незалежно від категорії."),
    ...
    case("TC-UI-PLANEXEC-016", "AC-02", "Excel-аркуші виконання дорівнюють видимим рядкам", "Застосувати категорію, пошук і Лише обрані при згорнутому out-of-plan та експортувати.", "За планом і Поза планом містять видимий ресурс та не містять відфільтровані; Розбір не обмежується цими фільтрами."),
