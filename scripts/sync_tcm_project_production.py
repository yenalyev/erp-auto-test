...
ACCEPTANCE_CRITERIA: dict[str, list[tuple[str, str]]] = {
    ...
    "REQ-OPER-HIST": [
        ...
        ("AC-18", "Виготовлене в CREATION обладнання має операцію «Виготовлено» у власній і загальній історії, відображається в таблиці обладнання та окремій картці «Обладнання (виготовлено)»; MODIFICATION не створює такої операції."),
...
CASES: list[dict[str, Any]] = [
    ...
    case("TC-UI-PROJ-HIST-001", "REQ-OPER-HIST", "AC-18", "Картка «Обладнання (виготовлено)» у загальній історії", "Завершити CREATION, відкрити загальну історію операцій і знайти створену одиницю за унікальним інвентарним номером.", "Видима окрема картка з точним заголовком «Обладнання (виготовлено)»; equipment-таблиця містить саме створену одиницю та операцію «Виготовлено» в тому самому рядку.", priority="CRITICAL", severity="CRITICAL"),
...
- На сторінці `/history` для операцій `PRODUCED` відображається окрема картка з точним заголовком «Обладнання (виготовлено)».
...
Підтверджений стан dev від 25.09.2026: `TC-UI-PROJ-HIST-001` пройшов 1/1. Після завершення `CREATION` картка «Обладнання (виготовлено)» видима, а створена одиниця присутня в equipment-таблиці за унікальним інвентарним номером з операцією «Виготовлено». Перевірка більше не допускає збіг лише за назвою моделі.
