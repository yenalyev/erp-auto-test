...
- Excel поточних видимих рядків у `За планом` і `Поза планом`: чинні шість колонок `За планом`, `Продукт | Категорія | Зроблено | Од. вимір | На складі (<дата>)` у `Поза планом`, header-only `Поза планом` за відсутності відфільтрованих рядків, поточну дату в `На складі` та згорнутий блок `Поза планом`; третій аркуш `Розбір` має `Продукт | Кількість` і не залежить від категорії, пошуку та обраних, а за відсутності даних розбору вибраної локації та періоду залишається порожнім;
...
- Станом на 25.09.2026 погоджено приймати фактичний контракт аркуша `Поза планом`: `Продукт | Категорія | Зроблено | Од. вимір | На складі (<дата>)`. Попереднє очікування `Од. виміру` після `Категорія` скасовано. Агрегація зміни unit (`2 шт + 3 кг` → один рядок `5 кг`) відповідає погодженому правилу.
- Повторний DEV-запуск `TC-API-PLANEXEC-002` від 25.09.2026 підтвердив новий заголовок і допустимий header-only стан `Поза планом`. Тоді сім рядків `Розбір` після порожнього category-filter вважалися невідповідністю; 26.09.2026 контракт уточнено: цей аркуш не фільтрується за категорією. Історичний результат: `docs/audits/2026-09-25/plan-execution-improvement-dev-test-report.md`.
- DEV-прогін 26.09.2026 до уточнення тесту: `TC-UI-PLANEXEC-015/016` пройшли, `TC-API-PLANEXEC-002` впав лише на застарілому очікуванні порожнього `Розбір`. Оновлений API-тест порівнює цей аркуш у нефільтрованому та двох категорійно відфільтрованих експортах.
- Повторний цільовий DEV-прогін після уточнення контракту: **3 тести, 3 passed, 0 failures/errors/skipped** (`TC-API-PLANEXEC-002`, `TC-UI-PLANEXEC-015/016`). API-перевірка підтвердила, що `Розбір` однаковий без категорійного фільтра та з ним. Після посилення `TC-UI-PLANEXEC-016` окремий DEV-прогін виявив іншу невідповідність: комбінація категорії, пошуку й «Лише обрані» повертає `Розбір` із заголовком і сімома порожніми рядками замість семи рядків із даними до фільтрації. Оновлений тест порівнює вміст після кожного фільтра окремо; два діагностичні перезапуски не дійшли до тесту через таймаут сторінки входу (`#username`, 60 с).
...
- `TC-UI-PLANEXEC-015` та `TC-API-PLANEXEC-002` актуалізовано під погоджений заголовок і порядок колонок `Поза планом`.
