...
| Зовнішнє отримання від постачальника | `REQ-EDIT_REL`; бухгалтерські атрибути партії — `REQ-EDIT_REL-008`, [окремий тест-план](RELOCATION_BATCH_ACCOUNTING_TEST_PLAN.md) |
...
Якщо `2151` відсутній у всьому ланцюжку предків або локація не має parent, нижньої часової межі немає: давнє переміщення можна створити, редагувати й видалити за наявності відповідних прав і допустимого статусу. Вік переміщення не обмежує право прийняти його, зокрема всередині TSUK-ієрархії. Водночас у діалозі «Прийняти» дату прийому можна вибрати лише від дати «Надіслано» до сьогодні включно; ця дата записується в «Отримано» окремо від дати відправлення. Деталі й сценарії — у [тест-плані дати отримання](RECEIVED_DATE_TEST_PLAN.md). В автотестах стабільний TSUK parent `2151` резолвиться через pool `TSUK_PARENT_UNITS`, а не дублюється як ID у сценаріях. Правило охоплює ресурси й обладнання; автотести ресурсних переміщень — `RelocationOneDayLimitTest`, `TC-REL-DATE-001…023`, та `TC-REL-091`. Кейси `018…021` окремо фіксують межі створення й редагування `FP1 → FP2` комірником TSUK-складу.
...
| `REQ-EDIT_REL / AC-08` — часова межа | TC-REL-DATE-001…021, TC-REL-091 | `RelocationOneDayLimitTest`, `RelocationInTransitEditTest` — обмеження для всіх нащадків TSUK, у тому числі `FP1 → FP2` під комірником; ієрархія без TSUK та null parent без нижньої межі; старе переміщення можна прийняти, вибравши дату прийому від «Надіслано» до сьогодні |
